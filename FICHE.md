...
## Prochaine étape
Le magnétisme du déroulé attend le verdict de l'utilisateur, amplifié une fois à sa demande. Son amplitude se règle en un endroit : les cinq variables `--aimant-*` en tête de la section MAGNÉTISME de `hub.css` (opacité de fond, échelle au loin, échelle au foyer, grossissement du titre, flou), plus `AIMANT_ECART` et `AIMANT_COLLE` dans `hub.js` pour la largeur de la vague et la force de l'aimantation. L'échelle au foyer ne doit pas dépasser 1,042, au-delà la page déborde à droite ; le flou est le seul poste qui coûte au processeur graphique, `--aimant-flou: 0` l'éteint.
